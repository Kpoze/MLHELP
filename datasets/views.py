...
from .forms import DatasetForm  # Assure-toi d'avoir un formulaire défini pour l'upload
...
def detail_dataset(request, dataset_id, workspace_id):
    ...
    # Passer les colonnes et les lignes sous forme de listes
...
# Créer une liste des données avec le nom de la colonne et le type pour chaque cellule
    ...
    return render(request, 'datasets/detail_dataset.html', {
        'dataset': dataset,
        'columns': columns,
        'column_types': column_types,  # Ajout des types de colonnes
...
def update_cell(request, workspace_id, dataset_id):
    if request.method == "POST":
        ...
        try:
            ...
            print(f"Valeur actuelle à ({row_input}, {column_input}): {df.iat[row_input, column_input]}")
            ...
            if column_type == 'object' or np.issubdtype(type(converted_value), column_type):
                ...
                df.to_csv(dataset.file.path, index=False)  # Sauvegarder le DataFrame
